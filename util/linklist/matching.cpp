string Ipath(ROW* map, int iStart, int jStart, int iEnd, int jEnd) {
    string Iline = "";
...
string Zpath(ROW* map, int iStart, int jStart, int iEnd, int jEnd) {
    string Iline, Lline;
...
string Upath(ROW* map, int mapWidth, int mapHeight, int iStart, int jStart, int iEnd, int jEnd) {
    string Iline, Lline;
...
string findPath(ROW* map, int mapWidth, int mapHeight, int iStart, int jStart, int iEnd, int jEnd) {
    if ( AccessMap(map, jStart, iStart) == AccessMap(map, jEnd, iEnd) && (iStart != iEnd || jStart != jEnd) ) {
...
bool isImpossible(ROW* map, int mapWidth, int mapHeight) {
    for (int i=1; i<mapHeight-1; i++) {
        for (int j=1; j<mapWidth-1; j++) {
            if (AccessMap(map, j, i) != 0) {
                for (int k=1; k<mapHeight-1; k++) {
                    for (int l=1; l<mapWidth-1; l++) {
                        if (AccessMap(map, l, k) != 0) {
...
void Shuffle(ROW* map, int mapWidth, int mapHeight) {
    int temp[128];
    int index = 0;
    for (int i=1; i<mapHeight-1; i++) {
        for (int j=1; j<mapWidth-1; j++) {
            if (AccessMap(map, j, i) != 0) {
                temp[index] = AccessMap(map, j, i);
                index++;
                ChangeMap(map, j, i, 0);
            }
        }
    ...
    for (int i=0; i<index; i++) {
        do {
            xRan = rand()%(mapWidth-2) + 1;
            yRan = rand()%(mapHeight-2) + 1;
        } while (AccessMap(map, xRan, yRan) != 0);
        ChangeMap(map, xRan, yRan, temp[i]);
    }
}

bool isOver(ROW* map, int mapWidth, int mapHeight) {
...
void Move(ROW* map, int mapWidth, int mapHeight, int &xCur, int &yCur, int &iStart, int &jStart, int &iEnd, int &jEnd, bool &swch, int levelChoice, bool &saveGame) {
    char ch = getch();
    if (ch == 72 || ch == 'w') {
        if (yCur == 1) {
            yCur = mapHeight - 2;
        ...
        for (int i=0; i<mapHeight; i++) {
            for (int j=0; j<mapWidth; j++) {
                loginPlayer.state[levelChoice].board[index] = AccessMap(map, j, i);
                index++;
            }
        }
