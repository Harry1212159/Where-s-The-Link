...
ROW* GenerateMap(int mapWidth, int mapHeight, int charNum) {
...
// Draw the map
void DrawCell(ROW* map, string* bg, int x, int y, int xCur, int yCur, int iStart, int jStart) {
    int cellWidth = 5, cellHeight = 3;
    if (AccessMap(map, x, y) != 0) {
        if (x == xCur && y == yCur) {
            SetColor(0, 15);
            GoTo(cellWidth*x, cellHeight*y);
            cout << char(201) << char(205) << char(205) << char(205) << char(187);
            GoTo(cellWidth*x, cellHeight*y+1);
            cout << char(186) << char(32) << char(64+AccessMap(map, x, y)) << char(32) << char(186);
            ...
            if (AccessMap(map, x, y) == 7) { // White
                SetColor(0, 13);
            } else if (AccessMap(map, x, y) == 8) { // Gray
                SetColor(0, 14);
            } else {
                SetColor(0, AccessMap(map, x, y));
            }
            GoTo(cellWidth*x, cellHeight*y);
            cout << char(218) << char(196) << char(196) << char(196) << char(191);
            GoTo(cellWidth*x, cellHeight*y+1);
            cout << char(179) << char(32) << char(64+AccessMap(map, x, y)) << char(32) << char(179);
            GoTo(cellWidth*x, cellHeight*y+2);
...
void Normal(Level level, int xCur, int yCur, int &iStart, int &jStart, int &iEnd, int &jEnd, int &score, bool &swch) {
    ...
    if (path != "") {
        ChangeMap(level.Map, jStart, iStart, 0);
        ChangeMap(level.Map, jEnd, iEnd, 0);
...
void Falldown(Level level, int xCur, int yCur, int &iStart, int &jStart, int &iEnd, int &jEnd, int &score, bool &swch) {
    ...
    if (path != "") {
        ...
        for (int i=iStart; i>1; i--) {
            int temp = AccessMap(level.Map, jStart, i);
            ChangeMap(level.Map, jStart, i, AccessMap(level.Map, jStart, i-1));
            ChangeMap(level.Map, jStart, i-1, temp);
        }
        for (int i=iEnd; i>1; i--) {
            int temp = AccessMap(level.Map, jEnd, i);
            ChangeMap(level.Map, jEnd, i, AccessMap(level.Map, jEnd, i-1));
            ChangeMap(level.Map, jEnd, i-1, temp);
        }
    }
    if (swch) {
        iStart = jStart = iEnd = jEnd = 0;
